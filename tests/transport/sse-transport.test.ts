  ...
  describe('SSE Initialization Functionality', () => {
    ...
    describe('Error Handling During Initialization', () => {
      ...
      it('should handle timeout during initialization process', (done) => {
        // Create a handler with shorter timeout for faster test
        const timeoutHandler = new SSETransportHandler({
          heartbeatInterval: 10000, // Long heartbeat to avoid interference
          connectionTimeout: 30000,
          maxConnections: 5,
          enableLogging: true,
          autoInitialize: true,
          initializationTimeout: 500 // Short timeout for testing
        });
        
        const mockHandlerFn = jest.fn();
        mockHandlerFn.mockImplementation(async (request: MCPRequest) => {
          if (request.method === 'initialize') {
            return new Promise(() => {}); // Never resolves
          }
          return { jsonrpc: '2.0', id: request.id, result: {} };
        });
        timeoutHandler.setRequestHandler(mockHandlerFn);
        ...
        // Timeline: 200ms autoInit delay + 500ms timeout = 700ms total
        setTimeout(() => {
        ...
        }, 1000); // Wait 1s (200 + 500 + 300 buffer)
      }, 5000);
